...
public class MessagingOrchestrator {
    ...
    private String buildLastMessage(List<Attachment> attachments, String messageText) {
        ...
        if (!attachments.isEmpty()) {
            LinkedHashMap<String, Integer> mimeTypeCount = new LinkedHashMap<>();

            for (var attachment : attachments) {
                String mimeType = attachment.getMimeType();
                if (mimeType.contains("image")) {
                    mimeTypeCount.merge("Фото", 1, Integer::sum);
                } else if (mimeType.contains("video")) {
                    mimeTypeCount.merge("Видео", 1, Integer::sum);
                } else {
                    mimeTypeCount.merge("Фаил", 1, Integer::sum);
                }
            }

            int index = 0;
            for (Map.Entry<String, Integer> entry : mimeTypeCount.reversed().entrySet()) {
                index++;
